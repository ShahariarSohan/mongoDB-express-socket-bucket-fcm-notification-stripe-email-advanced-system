...
const createIntentInStripe = async (payload: payloadType, userId: string, language: SupportedLanguage = 'en') => {
  ...
  const completePayment = { success: true };
...
const getSaveCardsFromStripe = async (userId: string, language: SupportedLanguage = 'en') => {
...
const deleteCardFromStripe = async (userId: string, last4: string, language: SupportedLanguage = 'en') => {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user) {
    throw new ApiError(404, "User not found!");
  }
...
const refundPaymentFromStripe = async (id: string, language: SupportedLanguage = 'en') => {
  // const findPayment = await prisma.payment.findUnique({
  //   where: {
  //     serviceId: id,
  //   },
  // });
  const findPayment: any = null;
  if (!findPayment) {
...
const cancelSubscriptionFromStripe = async (payload: { userId: string }, language: SupportedLanguage = 'en') => {
  const findUser = await prisma.user.findUnique({
    where: {
      id: payload.userId,
    },
